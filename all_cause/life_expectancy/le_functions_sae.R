...
set_starting_ax <- function(mx, sex, x) {
  ax <- rep(2.5, x)
...
calc_qx <- function(mx, ax, n, x, adjust_ax = F) {
  ...
  if (adjust_ax) {
    ...
    while (length(tobig) > 0) {
      if (min(ax[tobig]) <= 0.1) stop("qx > 1 and ax is already close to 0")
...
## Wang extrapolation.
lifetable <- function(mx, ax = NULL, sex = NULL, graduation = T, extrap = T) {
  ...
  if (is.null(ax)) ax <- set_starting_ax(mx, sex, x) 
